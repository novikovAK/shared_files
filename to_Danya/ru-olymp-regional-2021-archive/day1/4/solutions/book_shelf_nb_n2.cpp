...
void solve() {
	...
	if (!fl) {
		for (int i = 0; i < 2; ++i) {
			for (int j = 0; j < n; ++j) {
				if (i != j) {
					dist_vars.push_back({books[i].ss + books[j].ff});
				}
			}
		}
		// cerr << szof(dist_vars) << endl;
		// sort(dist_vars.begin(), dist_vars.end());
		// dist_vars.erase(unique(dist_vars.begin(), dist_vars.end()), dist_vars.end());
	} else {
		dist_vars = {dist};
	}
